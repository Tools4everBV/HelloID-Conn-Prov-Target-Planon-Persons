#################################################
# HelloID-Conn-Prov-Target-Planon-Persons-Delete
# PowerShell V2
#################################################
...
try {
    ...
    # If the account exists, it will be deleted. If the account does not exist, it will be logged in the audit log that it could not be found, possibly indicating that it could be deleted
    ...
    switch ($action) {
        'DeleteAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Delete Planon account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatUpdateParams
            }
            else {
                Write-Information "[DryRun] Delete Planon account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
        ...
        if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
            ...
            $auditMessage = "Could not Delete Planon account. Error: $($errorObj.FriendlyMessage)"
            Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
        }
        else {
            $auditMessage = "Could not Delete Planon account. Error: $($ex.Exception.Message)"
