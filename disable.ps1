...
try {
    ...
    # If the account exists, it will be disabled. If the account does not exist, it will be logged in the audit log that it could not be found, possibly indicating that it could be deleted
    ...
    switch ($action) {
        'DisableAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Dsiable Planon account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatUpdateParams
            }
            else {
                Write-Information "[DryRun] Disable Planon account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
