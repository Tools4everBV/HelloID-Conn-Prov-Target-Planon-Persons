##################################################
# HelloID-Conn-Prov-Target-Planon-Persons-Disable
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DisableAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Disabling Planon-Persons account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatDisableParams

            } else {
                Write-Information "[DryRun] Disable Planon-Persons account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not disable Planon-Persons account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not disable Planon-Persons account. Error: $($_.Exception.Message)"
